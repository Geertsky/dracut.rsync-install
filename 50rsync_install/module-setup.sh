...
install() {
	...
	PYTHONHASHSEED=42 $moddir/python-deps $moddir/dalco-check_and_format_disks.py $moddir/dalco-multicast_receiver.py $moddir/sendmail.py| while read dep; do
        	case "$dep" in
        	    *.so) inst_library $dep ;;
        	    *.py) inst_simple $dep ;;
		    dalco-check_and_format_disks.py) ;;
        	    *) inst $dep ;;
        	esac
	done
	inst $moddir/dalco-check_and_format_disks.py /lib/dalco/dalco-check_and_format_disks.py
	inst $moddir/dalco-multicast_receiver.py /lib/dalco/dalco-multicast_receiver.py
